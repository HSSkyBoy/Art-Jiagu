...
public class ValueChunk extends Chunk<Chunk.EmptyHeader> {
    ...
    private void evaluateExplicitType(String explicitTypeName, String value) {
        String normalized = explicitTypeName.toLowerCase(Locale.ROOT);
        switch (normalized) {
            case "bool":
            case "boolean":
                ...
                type = ValueType.INT_BOOLEAN;
                data = Boolean.parseBoolean(value) ? 1 : 0;
                return;
            case "int":
            case "integer":
                type = ValueType.INT_DEC;
            ...
            case "hex":
                type = ValueType.INT_HEX;
                data = (int) Long.parseLong(value.replaceFirst("^(?i)0x", ""), 16);
                return;
            case "float":
                type = ValueType.FLOAT;
                data = Float.floatToIntBits(Float.parseFloat(value));
                return;
            case "color":
                type = ValueType.INT_COLOR_ARGB8;
                data = Color.parseColor(value);
                return;
            case "dimen":
            case "dimension":
                type = ValueType.DIMENSION;
                data = evalcomplex(value);
                return;
            case "fraction":
                type = ValueType.FRACTION;
                data = evalcomplex(value);
            ...
            case "reference":
                type = ValueType.REFERENCE;
                data = getReferenceResolver().resolve(this, value);
                return;
            case "null":
                type = ValueType.NULL;
                data = 0;
                return;
            default:
                throw new IllegalArgumentException("Unsupported explicit value type: " + explicitTypeName);
        }
    }
}
